...
// PATCHED: catalog.data.gov retired its CKAN /api/3 endpoint; this uses the new /search API
// and maps results back into the CKAN package shape the original tools returned.
const BASE_URL = 'https://catalog.data.gov';
const toCkanPackage = (r) => ({
    id: r.identifier,
    name: r.slug,
    title: r.title,
    notes: r.description,
    url: `https://catalog.data.gov/dataset/${r.slug}`,
    organization: { title: r.publisher || r.organization?.name, name: r.organization?.slug },
    tags: (r.keyword || []).map((name) => ({ name })),
    resources: (r.dcat?.distribution || []).map((d) => ({
        name: d.title,
        format: d.format || d.mediaType,
        url: d.downloadURL || d.accessURL,
    })),
    metadata_modified: r.dcat?.modified || r.last_harvested_date,
});
/**
 ...
 */
const isValidPackageSearchArgs = (args) => typeof args === 'object' &&
    args !== null &&
 ...
 * @returns True if the arguments are valid, false otherwise.
 */
const isValidPackageShowArgs = (args) => typeof args === 'object' && args !== null && typeof args.id === 'string';
/**
 * Checks if the given arguments are valid for the group_list tool.
 * @param args - The arguments to check.
 * @returns True if the arguments are valid, false otherwise.
 */
const isValidGroupListArgs = (args) => typeof args === 'object' &&
    args !== null &&
 ...
 * @returns True if the arguments are valid, false otherwise.
 */
const isValidTagListArgs = (args) => typeof args === 'object' &&
    args !== null &&
...
class DataGovServer {
            ...
            tools: [
                {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            start: {
                                type: 'number',
                                description: 'Starting offset for results (unsupported by the new catalog API; use `after`)',
                            },
                            after: {
                                type: 'string',
                                description: 'Pagination cursor returned as `after` by the previous search',
                            },
                        },
                    },
                },
            ...
            switch (request.params.name) {
                case 'package_search':
                    if (!isValidPackageSearchArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, 'Invalid package_search arguments');
                    }
                    return this.packageSearch(request.params.arguments);
                case 'package_show':
                    if (!isValidPackageShowArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, 'Invalid package_show arguments');
                    }
                    return this.packageShow(request.params.arguments);
                case 'group_list':
                    if (!isValidGroupListArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, 'Invalid group_list arguments');
                    }
                    return this.groupList(request.params.arguments);
                case 'tag_list':
                    if (!isValidTagListArgs(request.params.arguments)) {
                        throw new McpError(ErrorCode.InvalidParams, 'Invalid tag_list arguments');
                    }
     ...
     */
    async packageSearch(args) {
        try {
            const params = { q: args.q, per_page: args.rows ?? 10 };
            if (args.after) params.after = args.after;
            const response = await this.axiosInstance.get('/search', { params });
            const results = (response.data.results || []).map(toCkanPackage);
            const body = { success: true, result: { count: results.length, results, after: response.data.after } };
            return {
                content: [
     ...
     */
    async packageShow(args) {
        try {
            const response = await this.axiosInstance.get('/search', {
                params: { q: args.id, per_page: 10 },
            });
            const hits = response.data.results || [];
            const hit = hits.find((r) => r.slug === args.id) || hits[0];
            if (!hit) {
                return { content: [{ type: 'text', text: `Dataset not found: ${args.id}` }], isError: true };
            }
            return {
                content: [
                    { type: 'text', text: JSON.stringify({ success: true, result: toCkanPackage(hit) }, null, 2) },
                ],
            };
        }
        catch (error) {
